...
const Starter = () => {
  ...
  return (
    <SafeAreaView style={{ flex: 1, ...tailwind('bg-white dark:bg-slate-900') }}>
      <StatusBar style={theme === 'light' ? 'dark' : 'light'} />
      <View style={tailwind('absolute top-14 right-4')}>
        <TouchableOpacity
          style={{
            ...tailwind('relative bg-white border border-slate-200 dark:border-transparent dark:bg-slate-800 py-2 px-3 rounded-md'),
          ...
          onPress={() => console.log('toggleTheme')}
        >
          {theme === 'light' ? (
            <View style={tailwind('flex-row items-center')}>
              <Sun width={20} height={20} fill={colors.yellow[400]} />
              <Text style={tailwind('text-base text-slate-600 ml-2')}>Clair</Text>
            </View>
          ) : (
            <View style={tailwind('flex-row items-center')}>
        ...
        <View style={tailwind('pt-20 items-center justify-center h-full')}>
          ...
          <View style={tailwind('mt-5')}>
            ...
            <View style={tailwind('items-center justify-center flex-row mt-3')}>
              {LOCALES.map(item => (
                <TouchableOpacity key={item.id} onPress={() => changeLocale(item.id)} style={tailwind('mx-2 px-3 py-2 relative overflow-hidden bg-white border border-slate-200 dark:bg-slate-800 dark:border-transparent rounded-md')}>
                  <Text style={tailwind('text-slate-500 dark:text-white text-center leading-5 font-medium')}>{translate(item.name)}</Text>
                  {(locale && locale === item.id) && <View style={tailwind('absolute bottom-0 left-0 right-0 h-1 bg-yellow-400')} />}
                </TouchableOpacity>
              ))}
            </View>
          </View>
